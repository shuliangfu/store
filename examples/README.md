...
这个示例展示了传统的状态管理方式，使用 `set`、`get` 和 `api` 参数来更新和访问状态。
...
- 新项目开发（推荐）
- 需要更简洁代码的场景
- 类似 Vue 3 Composition API 的使用习惯
...
1. **方法定义**：必须使用普通函数（`function`），不能使用箭头函数（`() => {}`），这样才能正确绑定 `this`
2. **类型定义**：建议为 Store 定义完整的接口类型，以获得更好的类型推断
3. **状态更新**：使用 `this` 方式时，属性修改会自动同步到 store，无需手动调用 `set()`
